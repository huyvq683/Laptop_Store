...
public class RamRepository {
    ...
    public Ram getOneRam(String ma) {
        String sql = " FROM Ram WHERE Ten = :ten ";
        Session session = HibernateUtil.getFACTORY().openSession();
        Query query = session.createQuery(sql, Ram.class);
        query.setParameter("ten", ma);
        Ram cpu = (Ram) query.getSingleResult();
        return cpu;
    }

    public Ram getOne(String maSP) {
        try (Session session = HibernateUtil.getFACTORY().openSession()) {
    ...
    public Boolean update(Ram sp, UUID id) {
        Transaction tran = null;
        try (Session session = HibernateUtil.getFACTORY().openSession()) {
            tran = session.beginTransaction();
            Ram s = session.get(Ram.class, id);
            s.setMa(sp.getMa());
            s.setTen(sp.getTen());
